...
function FormField({name, formik}) {
    let onChange = (field) => (e) => {
        formik.setFieldValue(field, e.target.value)
    };
    return <TextField placeholder={name} value={formik.values[name]} onChange={onChange(name)}/>;
}

export function EditPlanetModal({onClose, planet}) {
    ...
    >
        ...
        <><ModalHeader
            toggle={onClose}
        >
            Edit {planet.name} Planet
        </ModalHeader>
            <form onSubmit={formik.handleSubmit}><ModalBody>
